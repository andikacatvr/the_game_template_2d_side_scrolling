...
export class Scene4 extends Phaser.Scene {
    ...
    create() {
        ...
        // Tanah Dasar
        this.buildTiledGround(0, 434, 44);

        // Platform-platform Bertingkat
        this.buildTiledPlatform(300, 330, 4);
        this.buildTiledPlatform(550, 270, 4);
        ...
        // 3. Pintu Kembali (ke Scene 3) & Pintu Maju (ke Scene 5)
        ...
        const pbTxt = this.add.text(0, 0, '[E] Scene 3', { fontSize: '10px', fontStyle: 'bold', fill: '#cbd5e1', fontFamily: FONT_BODY }).setOrigin(0.5);
        this.promptBack.add([pbPill, pbTxt]);
        pbPill.setInteractive({ useHandCursor: true }).on('pointerdown', () => this.goToScene3());

        this.doorNext = this.physics.add.staticSprite(1330, 395, 'skeleton_portal');
        this.tweens.add({ targets: this.doorNext, alpha: 0.8, yoyo: true, repeat: -1, duration: 1000 });
        this.promptNext = this.add.container(1330, 350).setDepth(25).setVisible(false);
        const pnPill = this.add.rectangle(0, 0, 96, 20, 0x0c2744, 0.95).setStrokeStyle(1.5, 0x38bdf8);
        const pnTxt = this.add.text(0, 0, '[E] Scene 5', { fontSize: '10px', fontStyle: 'bold', fill: '#38bdf8', fontFamily: FONT_BODY }).setOrigin(0.5);
        this.promptNext.add([pnPill, pnTxt]);
        pnPill.setInteractive({ useHandCursor: true }).on('pointerdown', () => this.goToScene5());

        // 4. Karakter Pemain
        const spawnX = this.startData.fromScene5 ? 1270 : 100;
        ...
        this.showFloatingToast('SCENE 4: TEBING BERSALJU (TEMPLATE KOSONGAN)', 0x38bdf8);
    ...
    createHUD() {
        this.hud = new GameHUD(this, {
            sceneTitle: 'SCENE 4 - TEBING BERSALJU',
