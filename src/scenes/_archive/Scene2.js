...
export class Scene2 extends Phaser.Scene {
    ...
    create() {
        ...
        // 1. Batas Kamera & Fisika Dunia Lebar
        this.cameras.main.setBounds(0, 0, WORLD_WIDTH, WORLD_HEIGHT);
        this.physics.world.setBounds(0, 0, WORLD_WIDTH, WORLD_HEIGHT);
        this.cameras.main.setBackgroundColor('#7dd3fc'); // Biru muda cerah (sky blue)

        // 3. Platform & Rintangan
        this.platforms = this.physics.add.staticGroup();
        this.hazards = this.physics.add.staticGroup();

        // Tanah panjang dari awal sampai akhir (dengan sedikit jurang tantangan)
        this.buildTiledGround(0, 434, 28);       // x: 0 - 896
        this.buildTiledGround(960, 434, 33);     // x: 960 - 2000 (ada jurang di 896-960)

        // Platform-platform eksplorasi melayang
        this.buildTiledPlatform(320, 330, 4);
        this.buildTiledPlatform(580, 260, 5);
        this.buildTiledPlatform(920, 310, 4);    // Platform jembatan di atas jurang
        this.buildTiledPlatform(1220, 270, 4);
        ...
        // Pintu Masuk / Backtrack ke Scene 1
        ...
        const pbTxt = this.add.text(0, 0, '[E] Scene 1', { fontSize: '10px', fontStyle: 'bold', fill: '#cbd5e1', fontFamily: FONT_BODY }).setOrigin(0.5);
        this.promptBack.add([pbPill, pbTxt]);
        pbPill.setInteractive({ useHandCursor: true }).on('pointerdown', () => this.goToScene1());

        // Pintu Portal Maju ke Scene 3 (di ujung kanan dunia)
        this.doorNext = this.physics.add.staticSprite(1920, 395, 'skeleton_portal');
        this.tweens.add({ targets: this.doorNext, alpha: 0.8, yoyo: true, repeat: -1, duration: 1000 });
        this.promptNext = this.add.container(1920, 350).setDepth(25).setVisible(false);
        const pnPill = this.add.rectangle(0, 0, 96, 20, 0x0c2744, 0.95).setStrokeStyle(1.5, 0x38bdf8);
        const pnTxt = this.add.text(0, 0, '[E] Scene 3', { fontSize: '10px', fontStyle: 'bold', fill: '#38bdf8', fontFamily: FONT_BODY }).setOrigin(0.5);
        this.promptNext.add([pnPill, pnTxt]);
        pnPill.setInteractive({ useHandCursor: true }).on('pointerdown', () => this.goToScene3());

        // 5. Spawn Karakter Pemain
        const spawnX = this.startData.fromScene3 ? 1860 : 100;
        this.player = this.physics.add.sprite(spawnX, 370, 'skeleton_player').setDepth(10);
        this.player.setCollideWorldBounds(true);
        this.physics.add.collider(this.player, this.platforms);

        // Kamera Mengikuti Player (Smooth Follow + Deadzone)
        this.cameras.main.startFollow(this.player, true, 0.08, 0.08);
        this.cameras.main.setDeadzone(140, 70);

        // Overlap Duri Rintangan
        ...
        // 6. UI HUD Bersatu (Selaras dengan Scene 1)
        this.createHUD();
        ...
        // 8. Touch Controls untuk HP/Tablet
        this.createTouchControls();

        // Banner awal Scene
        this.showFloatingToast('SCENE 2: LEMBAH PANJANG (KAMERA MENGIKUTI PLAYER)', 0x38bdf8);
    ...
    createHUD() {
        this.hud = new GameHUD(this, {
            sceneTitle: 'SCENE 2 - LEMBAH PANJANG',
    ...
    takeDamage(amount) {
        ...
        this.showFloatingToast('-1 HP Terkena Rintangan!', 0xef4444);
    ...
    update() {
        ...
        // Cek kedekatan pintu untuk menampilkan prompt [E]
        ...
        if (this.player.y > 440) {
            this.takeDamage(1);
            this.player.setPosition(850, 250);
            this.player.setVelocity(0, 0);
        }
    }
}
